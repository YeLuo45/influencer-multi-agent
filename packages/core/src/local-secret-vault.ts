...
export function createLocalSecretVault(opts: { rootDir: string; passphrase?: string }): LocalSecretVault {
  ...
  return {
    ...
    get(key) {
      ...
      if (!raw) return null;
      return decodeSecret(raw, passphrase);
    },
    list() {
      const data = readVault(file, passphrase);
...
function encodeSecret(value: string, passphrase: string): string {
  ...
  const encoded = Array.from(value).map((char, index) => char.charCodeAt(0) ^ key.charCodeAt(index % key.length));
  return encoded.map((code) => code.toString(36).padStart(2, '0')).join('-');
}
