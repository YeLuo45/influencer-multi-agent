...
function pickPlatforms(angle) {
    const lower = angle.toLowerCase();
    const out = [];
    if (lower.includes('tech') || lower.includes('agent') || lower.includes('ai'))
        out.push('x', 'reddit', 'bilibili');
    if (lower.includes('种草') || lower.includes('测评') || lower.includes('生活'))
        out.push('xiaohongshu');
    if (lower.includes('热点') || lower.includes('娱乐'))
        out.push('weibo');
    if (out.length === 0)
        out.push('x', 'xiaohongshu');
    return out;
}
...
function parseIdeas(raw) {
    ...
    try {
        ...
        if (Array.isArray(data)) {
            return data.filter((x) => typeof x === 'object' && x !== null && typeof x.angle === 'string');
        }
    }
    catch {
        // fall through
