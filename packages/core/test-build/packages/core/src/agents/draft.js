...
export class DraftAgent {
    ...
    async run(input, content, ctx) {
        ...
        try {
            ...
            const title = await ctx.llm.complete(`topic: ${content.topic}\nidea angle: ${idea.angle}\nhook: ${idea.hook}\nGenerate a Chinese title.`, { maxTokens: 80 });
            const body = await ctx.llm.complete(`topic: ${content.topic}\ntitle: ${title}\nangle: ${idea.angle}\nWrite a 200-word Chinese post body.`, { maxTokens: 500 });
            const draft = {
                title: title.trim(),
                body: body.trim(),
