...
import { Pipeline, createContent } from '../src/index.js';
    ...
    return {
        llm: {
            async complete(prompt) {
                ...
                if (prompt.includes('Generate a Chinese title'))
                    return 'AI Agent 趋势：3 个被低估的真相';
                if (prompt.includes('Write a 200-word Chinese post body'))
                    return '我观察到 AI Agent 趋势已经全面爆发。\n\n为什么？三个信号：1) 流量结构变化，从搜索转向推荐；2) 用户行为分层，专业用户更愿意付费；3) 平台规则迭代，新算法对优质内容更友好。\n\n机会在哪？把老问题用新工具重做一次。普通玩家用 AI 工具降本，资深玩家用 AI 工具提质。\n\n#AI #大V观察 #趋势';
                return 'mock-llm ok';
            },
        },
        crawler: {
