...
import { TRANSITIONS, canTransition, assertTransition, isTerminal, InvalidTransition, } from '../src/state-machine.js';
import { CONTENT_STAGES } from '../src/types.js';
test('state-machine: every stage in CONTENT_STAGES', () => {
    for (const s of CONTENT_STAGES)
        assert.ok(TRANSITIONS[s], `missing transition for ${s}`);
});
