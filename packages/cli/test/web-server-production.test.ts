...
void test('web-server: /api/production exposes durable production operations snapshot', async () => {
  ...
  try {
    ...
    const json = await response.json() as {
      ...
      connectorExecution: { mode: string; steps: unknown[] };
      approvalStore: { rows: unknown[] };
      credentialHealthCenter: { cards: unknown[] };
      ciArtifactIngest: { mutatesRepo: boolean; gates: unknown[] };
      replayScenarios: { scenarios: unknown[] };
      eventTimeline: { events: unknown[]; failures: number };
      webModeEnhancements: Array<{ id: string }>;
    ...
    assert.equal(json.connectorExecution.mode, 'dry-run');
    assert.equal(json.connectorExecution.steps.length, 6);
    assert.equal(json.approvalStore.rows.length, 3);
    assert.equal(json.credentialHealthCenter.cards.length, 6);
    assert.equal(json.ciArtifactIngest.mutatesRepo, false);
    assert.equal(json.ciArtifactIngest.gates.length, 1);
    assert.equal(json.replayScenarios.scenarios.length, 5);
    assert.equal(json.eventTimeline.events.length >= 3, true);
    assert.equal(json.webModeEnhancements[0]?.id, 'guided-command-palette');
